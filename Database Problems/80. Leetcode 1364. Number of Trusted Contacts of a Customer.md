...
WITH T1 AS(
SELECT i.invoice_id, c.customer_name, i.price, i.user_id
FROM Invoices i LEFT JOIN Customers c ON i.user_id = c.customer_id
)

SELECT t1.invoice_id, t1.customer_name, t1.price, 
SUM(IF(c.contact_name IS NULL,0,1)) AS 'contacts_cnt', 
SUM(IF(c.contact_name IN (SELECT customer_name FROM Customers), 1, 0)) AS 'trusted_contacts_cnt'
FROM T1 t1 LEFT JOIN Contacts c ON t1.user_id = c.user_id
GROUP BY 1
ORDER BY 1

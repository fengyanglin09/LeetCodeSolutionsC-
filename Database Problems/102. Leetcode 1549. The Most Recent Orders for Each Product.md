...
WITH 
T AS (
SELECT ROW_NUMBER() OVER (PARTITION BY o.product_id ORDER BY o.order_date DESC) AS 'id',
o.order_id, o.product_id, o.order_date
FROM Orders o
)

SELECT p.product_name, t.product_id, t.order_id, t.order_date
FROM T t LEFT JOIN Products p ON t.product_id = p.product_id
WHERE t.id = 1 OR
t.order_date = (SELECT t1.order_date FROM T t1 WHERE t1.id = 1 AND t1.product_id = t.product_id)

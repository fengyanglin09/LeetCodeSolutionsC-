...
,
T2 AS(
SELECT t1.action_date, COUNT(DISTINCT(r.post_id))/COUNT(DISTINCT(t1.post_id)) AS 'remove_rate'
FROM T1 t1 LEFT JOIN Removals r ON t1.post_id = r.post_id
GROUP BY 1
)
SELECT ROUND(AVG(t2.remove_rate) * 100,2) AS 'average_daily_percent'
FROM T2 t2
